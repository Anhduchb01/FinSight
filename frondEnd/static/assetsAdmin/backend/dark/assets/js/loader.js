...
var switchFunctionality = {
    ...
    setMode: function() {
        ...
        if (Cookies.getCookie('dark_mode') != "") {
            ...
            var linkElement = document.querySelectorAll('link');
            var scriptElement = document.querySelectorAll('script');
            var imgLogo = document.querySelectorAll('img[alt="logo"]');

            for (let i = 0; i < imgLogo.length; i++) {
                if (imgLogo[i]) {
                    imgLogo[i].setAttribute(
                    "src",
                    "backend/dark/assets/img/logo-w-black.svg"
                  );
                }
              }
            ...
            for (var i = 0; i < linkElement.length; i++) {
                ...
                if( getHref == "backend/assets/css/loader.css" ) {
                } else {

                    if(n) {
                        linkElement[i].setAttribute('href', getHref);
                    } else {
                        linkElement[i].setAttribute('href', 'backend/dark/' + getHref.slice(8));
                    }
                }
            }
            ...
            for (var i = 0; i < scriptElement.length; i++) {
                ...
                if( getSrc == "backend/dark/assets/js/loader.js" ) {
                } else {
                    scriptElement[i].setAttribute('src', 'backend/dark/' + getSrc.slice(8));
                }
            }
