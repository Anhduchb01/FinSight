<template>
  <div>
    ...
    <div class="search">
      ...
      <input v-model="textSearch" v-on:keyup.enter="LoadDataInbox(1,textSearch)" type="text" class="form-control input-search" placeholder="Search Here..." />
      <button v-show="textSearch.length > 0" v-on:click="textSearch = ''" class="btn btn-primary search-button" style="position: absolute;
                                right: 8px;
                                top: 5px;
    ...
    <div class="action-center">
      ...
      <div class="tool-bar" v-if="showToolBarInbox">
        ...
        <div data-tooltip="Mark as unread" class="items-toolbar action-mark_as_unRead group-one" v-on:click="MarkUnread()">
          <div class="item-toolbar">
        ...
        <div data-tooltip="Spam" class="items-toolbar action-spam group-one" v-on:click="MarkToSpam()">
          <div class="item-toolbar">
            <svg style="margin-right: 0px;" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-alert-triangle">
        ...
        <div data-tooltip="Trash" class="items-toolbar action-delete group-one" v-on:click="MoveToTrash()">
          <div class="item-toolbar">
            <svg style="margin-right: 0px;color: #ff0000;" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <div class="tool-bar-pagination" v-if="resultsListContact.length">
    ...
    <div class="content-box" v-if="resultsDetail&&Object.keys(resultsDetail).length" style="width: 100%;left: 0px;right: 100%;">
      <div class="d-flex msg-close">
        <svg v-on:click="removeResultDetail()" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-arrow-left close-message">
      ...
      <div id="mailCollapseEleven" class="collapse ok show" aria-labelledby="mailHeadingEleven" data-parent="#mailbox-inbox">
    ...
    <div class="message-box" v-else>
        ...
        <div class="modal-dialog" role="document" style="margin-top: 15%;">
          <div class="modal-content" style="padding: 0px;">
            ...
            <div class="modal-footer">
              ...
              <button type="button" data-dismiss="modal" onclick="DeleteTrash()" class="btn btn-primary feather feather-trash permanent-delete">Yes</button>
            </div>
          </div>
        </div>
      </div>
      <div class="message-box-scroll" id="ct" v-if="resultsListContact&&resultsListContact.length">
        <div v-for="(result,i) in resultsListContact" :key="i" id="unread-verification-link" :class="'mail-item mailInbox num-'+i">
                  ...
                  <div class="d-flex">
                    <div class="n-chk text-center" style="width: 30px;">
                      <label class="new-control new-checkbox checkbox-primary" style="top:-5px;padding: 14px;padding-right: 0px;margin-bottom: -11px;left: -14px;">
                        <input @change="CheckToShow()" :value="result._id" v-model="ArrayChecked[i]" type="checkbox" class="new-control-input inbox-chkbox" name="check" ref="theCheckbox" />
                        <span class="new-control-indicator" style="top: 0px"></span>
                      </label>
                    </div>
                    <div class="f-head" style="padding-left: 10px;">
                      <div class="avatar avatar-sm">
                        <span class="avatar-title rounded-circle">{{getNameInitials(result.customerName)}}</span>
                      </div>
                    </div>
                    <div class="f-body" v-on:click="getInformationDetail(result._id)" data-toggle="collapse" role="navigation" data-target="#mailCollapseEleven${results[i]._id}" aria-expanded="false">
                      <div class="meta-mail-time">
                        <div>
                          <span data-tooltip="That's Good" data-tooltip-location="right" style="
                                                  width: 15px;
                                                  height: 15px;
                                                  border-radius: 50%;
                                                  padding: 1px;
                      ...
                      <div class="meta-title-tag">
                        <p class="mail-content-excerpt" data-mail-description='{"ops":[{"insert":"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Morbi pulvinar feugiat consequat. Duis lacus nibh, sagittis id varius vel, aliquet non augue. Vivamus sem ante, ultrices at ex a, rhoncus ullamcorper tellus. Nunc iaculis eu ligula ac consequat. Orci varius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Vestibulum mattis urna neque, eget posuere lorem tempus non. Suspendisse ac turpis dictum, convallis est ut, posuere sem. Etiam imperdiet aliquam risus, eu commodo urna vestibulum at. Suspendisse malesuada lorem eu sodales aliquam.\n"}]}'>
                          <span class="mail-title" data-mail-title="${results[i].customerSub}">{{result.customerSub}} -</span>
                          {{result.customerMsg}}
                        </p>
                        <div class="tags"></div>
      ...
      <div class="message-box-scroll" v-else>
        <div class="mail-item mailInbox" style="text-align: center;
                                                display: flex;
                                                height: 87%;
          ...
          <h1 style="position: absolute; font-size: 22px;
            font-weight: normal;
            margin-top: 24px;
            font-size: 23px;margin-bottom: -43px;color:black">Maibox is empty</h1>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    showAllMessage() {
      if (!this.selectAll) {
        for (var i = 0; i < this.resultsListContact.length; i++) {
          this.ArrayChecked[i] = true;
        }
        this.showToolBarInbox = true;
      } else {
        for (var i = 0; i < this.resultsListContact.length; i++) {
          this.ArrayChecked[i] = false;
        }
        this.showToolBarInbox = false;
      }
    },
    CheckPagination(action) {
      if (action == "forward") {
        this.pageMessage += 1;
        this.LoadDataInbox(this.pageMessage, this.textSearch);
      }
      if (action == "back") {
        this.pageMessage -= 1;
        this.LoadDataInbox(this.pageMessage, this.textSearch);
    ...
    MarkToSpam() {
      ...
      this.idMessageRemove = Array.from(this.idMessageRemove);
      HTTP.post(`move-to-spam`, {
        data: this.idMessageRemove,
      }).then(() => {
        this.ArrayChecked = [];
        this.LoadDataInbox(1, this.textSearch);
        this.$root.$emit("LoadNotify");
        this.$root.$emit("LoadDataSpam");
        new Snackbar(`1 Mail Added to Spam`, {
    ...
    MoveToTrash() {
      this.getIdRemoveTrash();
      this.idMessageRemove = Array.from(this.idMessageRemove);
      HTTP.post(`move-to-trash`, {
        data: this.idMessageRemove,
      }).then(() => {
        this.ArrayChecked = [];
        this.LoadDataInbox(1, this.textSearch);
        this.$root.$emit("LoadNotify");
        this.$root.$emit("LoadDataTrash");
        new Snackbar(`1 Mail Deleted`, {
          position: "bottom-right",
          theme: "light",
          style: {
            container: [
    ...
    CheckToShow() {
      if (
        this.CheckCheckedAllTrue(this.ArrayChecked) &&
        this.ArrayChecked.length == this.resultsListContact.length
      ) {
        this.selectAll = true;
      } else {
        this.selectAll = false;
    ...
    getIdRemoveTrash() {
      this.idMessageRemove = new Set();
      for (var i = 0; i < this.ArrayChecked.length; i++) {
        if (this.ArrayChecked[i] == true) {
          this.idMessageRemove.add(this.resultsListContact[i]._id);
        }
      }
    },
    LoadDataInbox(page, key) {
      HTTP.get("list-contact", { params: { page: page, key: key } })
        .then((response) => {
          this.resultsListContact = response.data[0];
          this.$emit("LoadDataInbox", this.resultsListContact);
  ...
  mounted() {
    ...
    for (var i = 0; i < this.resultsListContact; i++) {
      this.ArrayChecked[i] = false;
    }
    //Get router detail 
