<template>
  <div>
    ...
    <div class="search">
      ...
      <input v-model="textSearch" v-on:keyup.enter="LoadDataSpam(1,textSearch)" type="text" class="form-control input-search" placeholder="Search Here..." />
    ...
    <div class="action-center">
      ...
      <div class="tool-bar" v-if="showToolBarSpam">
      ...
      <div class="tool-bar-pagination" v-if="resultsListSpam.length">
    ...
    <div class="message-box" v-else>
      ...
      <div class="message-box-scroll" id="ct" v-if="resultsListSpam&&resultsListSpam.length">
        <div v-for="(result,i) in resultsListSpam" :key="i" id="unread-verification-link" :class="'mail-item spam num-'+i">
                  ...
                  <div class="d-flex">
                    <div class="n-chk text-center" style="width: 30px;">
                      <label class="new-control new-checkbox checkbox-primary" style="top:-5px;padding: 14px;padding-right: 0px;margin-bottom: -11px;left: -14px;">
                        <input @change="CheckToShow()" v-model="ArrayChecked[i]" type="checkbox" :value="result._id" class="new-control-input inbox-chkbox" name="check" />
                    ...
                    <div class="f-body" v-on:click="getInformationDetail(result._id)" data-toggle="collapse" role="navigation" :data-target="'#mailCollapseEleven'+result._id" aria-expanded="false">
                      <div class="meta-mail-time">
                        <div class="tooltips">
                          <span style="width: 15px;height: 15px;border-radius: 50%;padding: 1px;font-size: 10px;color: #fff !important;" class="badge badge-success">{{result.scoreCaptcha}}</span>
      ...
      <div v-else class="mail-item spam" style="text-align: center;display: flex;height: 87%;justify-content: center;flex-flow: wrap-reverse;background: #f8f4bf;border-radius: 50%;width: 350px; height: 350px;                                    margin: 80px auto;opacity: 60%;">
        ...
        <h1 style="position: absolute; font-size: 22px;font-weight: normal;margin-top: 24px;font-size: 23px;margin-bottom: -43px;color:black">Great ! No spam here</h1>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    CheckPagination(action) {
      if (action == "forward") {
        this.pageMessage += 1;
        this.LoadDataSpam(this.pageMessage, this.textSearch);
      }
      if (action == "back") {
        this.pageMessage -= 1;
        this.LoadDataSpam(this.pageMessage, this.textSearch);
    ...
    showAllMessage() {
      if (!this.selectAll) {
        for (var i = 0; i < this.resultsListSpam.length; i++) {
          this.ArrayChecked[i] = true;
        }
        this.showToolBarSpam = true;
      } else {
        for (var i = 0; i < this.resultsListSpam.length; i++) {
          this.ArrayChecked[i] = false;
        }
        this.showToolBarSpam = false;
      }
    },
    CheckToShow() {
      if (
        this.CheckCheckedAllTrue(this.ArrayChecked) &&
        this.ArrayChecked.length == this.resultsListSpam.length
      ) {
        this.selectAll = true;
      } else {
    ...
    getIdRemoveTrash() {
      ...
      for (var i = 0; i < this.ArrayChecked.length; i++) {
        if (this.ArrayChecked[i] == true) {
          this.idMessageRemove.add(this.resultsListSpam[i]._id);
        }
      }
    },
    MoveToTrash() {
      ...
      HTTP.post(`move-to-trash`, {
        ...
        this.ArrayChecked = [];
        this.LoadDataSpam(1, this.textSearch);
    ...
    LoadDataSpam(page, key) {
      HTTP.get("contact-spam", { params: { page: page, key: key } })
        .then((response) => {
          this.resultsListSpam = response.data[0];
          this.$emit("LoadDataSpam", this.resultsListSpam);
  ...
  mounted() {
    ...
    for (var i = 0; i < this.resultsListSpam; i++) {
      this.ArrayChecked[i] = false;
    }
     //Get router detail
