...
const { getArticleHasTag, getYearArray,getTimeLineOfTag } = require("../../service/front-end/articleService");
...
router.get("/get-article-has-tag", async (req, res) => {
  try {
    ...
    if (myCache.has("get-article-has-tag-"+String(text))) {
      return res.send(myCache.get("get-article-has-tag-"+String(text)));
    } else {

      let numberPage =req.query.numberPage;
      const articles = await getArticleHasTag(text,numberPage);
      myCache.set("get-article-has-tag-"+String(text), articles)
...
router.get("/get-timeline-of-tag", async (req, res) => {
        try {
          ...
          if (myCache.has("get-timeline-of-tag-"+String(text))) {
            ...
            const timeline = await getTimeLineOfTag(text);
